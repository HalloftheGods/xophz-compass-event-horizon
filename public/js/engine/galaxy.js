...
function generateGalaxy() {
  ...
  for (var i = 0; i < count; i++) {
    ...
    if (Math.random() > 0.99)
      p.size *= Math.pow(1 + Math.random(), 3 + Math.random() * 3) * 0.9;
    else if (Math.random() > 0.7)
      p.size *= 1 + Math.pow(1 + Math.random(), 2) * 0.04;
  ...
  var pGalacticSystem = new THREE.ParticleSystem(
    pGalaxy,
    galacticShaderMaterial
  );
  ...
  attachLegacyMarker("Over 400 (Billion) Stars!", measurement, 1.0, {
    min: 6000,
    max: 120000,
  });
  ...
  // Length, Bubble, Width
  matrix.scale(new THREE.Vector3(1.5, 0.1, 1.5));
