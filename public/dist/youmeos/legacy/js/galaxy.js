var galacticTexture0 = THREE.ImageUtils.loadTexture(
  window.youmeosBaseUrl + "/images/galactic_sharp.png"
);
var galacticTexture1 = THREE.ImageUtils.loadTexture(
  window.youmeosBaseUrl + "/images/galactic_blur.png"
);
var galaxypng = THREE.ImageUtils.loadTexture(window.youmeosBaseUrl + "/images/galactictop.png");
...
function generateGalaxy() {
  window.setLoadMessage("Generating the galaxy");
  ...
  for (var i = 0; i < count; i++) {
    ...
    if (Math.random() > 0.3) sa *= (1 + Math.random()) * 4;
    x += random(-sa, sa);
    z += random(-sa, sa);

    var distanceToCenter = Math.sqrt(x * x + z * z);
    var thickness =
      constrain(Math.pow(constrain(90 - distanceToCenter * 0.1, 0, 100000), 2) * 0.02, 2, 10000) +
      Math.random() * 120;
    ...
    if (Math.random() > 0.99) p.size *= Math.pow(1 + Math.random(), 3 + Math.random() * 3) * 0.9;
    else if (Math.random() > 0.7) p.size *= 1 + Math.pow(1 + Math.random(), 2) * 0.04;
    ...
    var r = constrain(1 - Math.pow(dist, 3) * 0.00000002, 0.3, 1) + random(-0.1, 0.1);
    var g = constrain(0.7 - Math.pow(dist, 2) * 0.000001, 0.41, 1) + random(-0.1, 0.1);
  ...
  window.toggleGalacticMeasurement = function (desired) {
    if (desired == undefined) pGalacticSystem.measurement.visible = !this.measurement.visible;
  ...
  // Length, Bubble, Width
  matrix.scale(new THREE.Vector3(1.5, 0.001, 1.5));
