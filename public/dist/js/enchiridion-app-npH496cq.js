...
* **Default State:** \`DIY \${tier.name.toUpperCase()}BOX (COMING SOON)\` with icon \`fal fa-box-usd\`.
* **Hover State:** \`WHAT'S IN THE BOX?\` with icon \`fal fa-box-open\`.
...
## Stripe REST Connector Integration

The frontend calls the WordPress REST endpoint:
...
\`\`\`json
{
  ...
  "success_url": "https://example.com/wp-admin/admin.php?page=xophz-compass#/checkout_success",
  "cancel_url": "https://example.com/wp-admin/admin.php?page=xophz-compass#/tesseract"
}
\`\`\`
* **Response:**
\`\`\`json
...
\`\`\`
* If the Stripe API key is not configured in WordPress settings, the endpoint returns a mock checkout redirect URL for local development and testing.
