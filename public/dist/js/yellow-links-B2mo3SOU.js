...
├── components/
│   ├── BubblegumShareModal.vue (Workspace Share Modal)
│   ├── BubblegumCollaboratorListItem.vue (Collaborator Row Molecule)
│   ├── BubblegumShareAccessSettings.vue (Access Tier Controls Molecule)
│   ├── BubblegumShareInviteForm.vue (User Autocomplete & Invite Form Molecule)
│   ├── BubblegumSandboxSplash.vue (Zero-State Workspace Canvas Splash Molecule)
│   └── BubblegumAiGeneratorModal.vue (20 Questions AI Scoping Wizard & Braindump Modal)
├── composables/
│   ├── useBubblegumState.ts (Unified Master Reactive State Orchestrator)
│   ├── useBubblegumPacks.ts (Pack Lifecycle, Sync, Import/Export & Stats)
│   ├── useBubblegumTaskOperations.ts (Task Actions, Toughness, Chewing & Categorization)
│   ├── bubblegumCharts.ts (Velocity & Doughnut Telemetry Generators)
│   ├── bubblegumPrompt.ts (AI MoSCoW Prompt Synthesizer)
│   └── useBubblegumAiWizard.ts (20 Questions AI Scoping Machine & Task Synthesizer)
└── views/
...
### 2.4 AI Task Generator (20 Questions Scoping Wizard)
- **Modal Capsule**: \`<bubblegum-ai-generator-modal>\` provides an intelligent scoping wizard powered by \`useBubblegumAiWizard\`.
- **Question Progression**:
  1. *Seed Prompt (Question 1)*: Initial user goal or concept with speech dictation support (\`m-dictation-btn\`).
  2. *Adaptive Clarifications (Questions 2 to 20)*: Gemini generates up to 19 targeted multiple-choice follow-ups probing technical architecture, APIs, data ingestion, user roles, security, and edge cases.
  3. *Early Exit Synthesis*: Users can trigger "Generate Tasks Now" at any point once sufficient context is established, or follow recommended early completion flags.
  4. *Wait-State Visual Cues*: Interactive inputs fade (\`opacity: 0.15\`) during AI generation while centered glowing pulse icons, indeterminate progress gauges, and hardcoded cycling status messages keep operators informed.
  5. *Task Sizing & MoSCoW Prioritization*: Synthesizes structured markdown tasks with mandatory MoSCoW priorities \`(M), (S), (C), (W)\` and story point estimates \`<1>..<21>\`.
  6. *Quick Braindump Fallback*: Retains direct one-shot text ingestion via header mode switcher pill.
...
\`\`\`
`,T=`# Local Produce Spark Operating Manual
...
\`\`\`
`,E=`# Logos Spark Operating Manual
...
\`\`\`
`,I=`# Howl's Moving Castle Spark Operating Manual
...
\`\`\`
`,x=`# Music Stream Spark Operating Manual
...
apps/youmeos/
├── routes/noosphere/
│   ├── NoosphereDashboard.vue (Table of Contents View: < 180 lines)
│   ├── useNoosphere.ts (State management composable)
│   ├── noosphere-canvas.vue (Topography canvas background)
│   └── components/
│       ├── m-observer-hero.vue (Hero billboard, pulse-ring, telemetry stats, initialize trigger)
│       ├── m-initialize-sphere-dialog.vue (Obsidian dialog capsule with justify-space-between actions)
│       ├── m-global-pulse.vue (Stream feed with category squircles, upvoting, comments)
│       ├── m-your-spheres.vue (Synchronized spheres list, empty galaxy state, global tags, topography preview)
│       ├── types.d.ts (Co-located domain interfaces)
│       └── *.spec.ts (Co-located unit test suites)
└── app/sparks/noosphere/
...
### 2.2 Atomic Atoms & Molecules
- **The Observer Hero (\`m-observer-hero\`)**: Luminous telemetry billboard with animated pulse ring, responsive telemetry counters, and collision-free action triggers.
- **Initialize Sphere Dialog (\`m-initialize-sphere-dialog\`)**: Obsidian glassmodal with auto-generated slugs, two-stage boolean validation, and standard action layout.
- **Global Pulse Feed (\`m-global-pulse\`)**: Starship obsidian glass cards with squircle category icons, metadata chips, upvoting counters, and comment metrics.
- **Your Spheres & Topography (\`m-your-spheres\`)**: Multi-section panel featuring synchronized communities, rotating galaxy empty state, trending tag chips, and interactive mind map topography preview.
...
### 2.3 Styling & Visual Design System
- **Brand Palette**: Noosphere Violet (\`#7c3aed\`), Starship Cyan (\`#62c9ff\`), and Luminous Azure (\`#38bdf8\`) with dark cosmic styling.
...
\`\`\`
`,F=`# Nucleos Spark Operating Manual
...
\`\`\`
`,V=`# Omega Source Spark Operating Manual
...
└── components/
    ...
    ├── o-integration-card.vue (Adaptive 3D Flip Card for OAuth & Web3 Integrations)
    ├── o-links-tab.vue (Dynamic Sigmatures Array & Web Identity Grid)
    ├── m-sigmature-wizard-dialog.vue (Smart Platform Auto-Detection & Link Wizard)
...
- **Passport Card & Wallet**: Holographic badge and wallet cards configured in a balanced 50/50 responsive grid with strict overflow containment to prevent avatar ring bleeding.
- **Sigmatures Array & Wizard**: Dynamic multi-profile signature array supporting multiple profiles per platform (e.g. GitHub, LinkedIn, X, YouTube, Substack, Discord, Bluesky, Twitch, Instagram, Mastodon). Features an auto-detecting modal wizard that parses pasted URLs, normalizes handles, supports custom labels, and complies with self-cleaning timer discipline.
- **Integration Card**: Adaptive 3D flip card with container-query responsive layout, brand squircle icons, and permissions management.
- **Beta Access Keycards**: High-tech access cards featuring gold smartcard IC chips, wireless RFID wave indicators, clearance level typography, ambient radial glows in tier colors, and interactive terminal modal insertion.

### 2.3 Styling & Visual Design System
- **Brand Palette**: Cosmic Purple (\`#b388ff\`) and Electric Cyan (\`#62c9ff\`) with deep starfield nebulae.
- **Access Tier Palette**: Omega (\`#4caf50\`), Beta (\`#ffc107\`), Alpha (\`#f44336\`), and Chi (\`#62c9ff\`).
...
tags:
  ...
  - xp
  - youmeos
ai_summary: "Comprehensive architectural manual for THEOS (Omega Source), detailing unified Sovereign Passport data models, Web3 wallet integration, OAuth account linking, high-tech access keycards, and cosmic singularity shaders."
\`\`\`
...
\`\`\`
`;export{p as $,G as A,W as B,L as C,B as D,_ as E,x as F,I as G,O as H,A as I,R as J,D as K,M as L,P as M,E as N,T as O,C as P,f as Q,k as R,w as S,v as T,b as U,S as V,y as W,h as X,g as Y,m as Z,ue as _,de as a,u as a0,d as a1,c as a2,l as a3,s as a4,i as a5,r as a6,o as a7,a as a8,t as a9,n as aa,e as ab,ce as b,le as c,se as d,ie as e,re as f,oe as g,ae as h,te as i,ne as j,ee as k,$ as l,Z as m,J as n,X as o,Q as p,q,K as r,Y as s,j as t,H as u,U as v,z as w,V as x,F as y,N as z};
