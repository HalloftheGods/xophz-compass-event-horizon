...
//	Enable cross origin for all textures
...
function start(e) {
  ...
  if (!Detector.webgl) {
    ...
    gradientImage.crossOrigin = "Anonymous"; // Fix Canvas Taint
    gradientImage.onload = postStarGradientLoaded;
    gradientImage.src = window.youmeosBaseUrl + "/images/star_color_modified.png";
  }
}
...
var postShadersLoaded = function () {
  if (enableDataStar) {
    loadStarData(window.youmeosBaseUrl + "/data/stars_all.json", function (loadedData) {
...
function initScene() {
  ...
  camera.position.z = 57777;
  camera.rotation.vx = 0;
  camera.rotation.vy = 0;
  camera.position.target = { x: 0, z: 57777, pz: 2000 };
...
function displayIntroMessage() {
  ...
  tour
    .showMessage(
      '"To my children. Krislynn Night, Khronus Infinidee, & Gaia Rhæ."',
      5000,
      //  )
